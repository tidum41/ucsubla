...
export default function CreateListingV2Page() {
  ...
  // Prevents infinite loop in quarters ↔ dates bidirectional sync
  ...
  const validateStep = (step: number): Record<string, string> => {
    ...
    // Step 3 (amenities) — all optional, no validation
    ...
    <div className="min-h-screen bg-background app-container flex flex-col">
      ...
      <div className="blurHeaderCentered app-container">
        <div className="blurHeaderCenteredContent">
          <button
            onClick={handleBack}
            className="text-h3 text-uclaBlue font-medium w-[52px]"
          >
            {currentStep === 0 ? 'Cancel' : '← Back'}
          </button>
          <WizardProgressBar currentStep={currentStep} totalSteps={TOTAL_STEPS} />
          <span className="text-small text-slateGray w-[52px] text-right">
      ...
      {/* Step content — clipped for slide animation */}
      ...
      <div className="fixed bottom-[65px] left-0 right-0 app-container px-5 py-3 bg-white/90 backdrop-blur-sm border-t border-[#E2E8F0]">
        <div className="flex items-center gap-3">
          {currentStep > 0 ? (
            <button
              ...
              className="flex-1 border border-[#E2E8F0] rounded-[18px] py-3.5 text-body font-medium text-darkSlate active:scale-[0.98] transition-transform disabled:opacity-50"
            >
              Back
            </button>
          ) : (
          ...
          <button
            ...
            className="flex-1 bg-uclaBlue text-white rounded-[18px] py-3.5 text-body font-medium shadow-elevated active:scale-[0.98] transition-transform disabled:opacity-60 flex items-center justify-center gap-2"
          >
